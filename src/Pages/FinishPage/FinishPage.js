    ...
    GetPlayer(playerid)
      .then((res) => {
        ...
        res.data.answers.forEach((item) => {
          if (item.question.answer === item.answer) numberofGood++;
        });
    ...
    <div className="flex w-full justify-center  bg-yellow-300 ">
      <div className="flex flex-col w-3/4 justify-center items-center">
        <div className=" flex w-1/6 p-3 justify-center items-center rounded-full bg-slate-400">
          <img className="w-full h-full " src={man} alt="man" />
        </div>
        <button className="flex flex-col justify-center items-center md:flex-row w-full  md:w-5/6 mt-3 mb-3 bg-cyan-400 shadow-slate-700 shadow-md  text-2xl md:text-5xl hover:bg-slate-100 h-20 rounded-2xl ">
          {Player.name}{" "}
        ...
        <button
          onClick={() => Retry()}
          className="w-3/6 mt-3 mb-3 bg-green-400 shadow-slate-700 shadow-md text-5xl hover:bg-slate-100 h-14 rounded-2xl "
        >
          Retry
        </button>
      </div>
