...
const Question = ({ question }) => {
    ...
    const optionsRender = question.options.map((item, index) => {
        const isGoodAnswer = item === question.answer;
        return (<div className="flex my-2 space-x-3" key={index}>
            {isGoodAnswer ?
            ...
            <div className="w-full flex flex-col flex-wrap justify-center border-b p-4">
                <div className="flex justify-between">
                    ...
                    <div className="flex space-x-2">
                        <svg onClick={()=>{navigate(`/editquestion/${question._id}`)}} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 text-blue-500 cursor-pointer">
